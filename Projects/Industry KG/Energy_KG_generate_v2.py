    ...
    def create_node(self, label, **attributes):
        with self._driver.session() as session:
            ...
            for k, v in attributes.items():
                k = k.replace(' ', '_').replace(':', '_').replace('（', '_').replace('）', '_').replace('#', 'No').replace('/', '_').replace("\n", "_").replace("——", "_").replace(".", "_").replace(",", "_").replace('(', '_').replace(')', '_')   # replace space with underscore
                query += "{}:'{}',".format(k, v)
            query = query.rstrip(',')
            query += "}) RETURN a"
...
if __name__ == "__main__":
    ...
    df = session.extract_nodes_from_excel(energy_file_path, 'GUM-KG', 4)
