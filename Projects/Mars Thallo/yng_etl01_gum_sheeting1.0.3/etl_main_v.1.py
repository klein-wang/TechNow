...
def spc_cur_process(now, df_T_spc, df_cur_spc): 
    """  
    更新spc最近60分钟的值到df_cur，
    """

    #筛选称重的记录
    df_T_spc = df_T_spc[(df_T_spc["EntryType"] == 2) | (df_T_spc["EntryType"] == 3)]

    #如果spc称重有数据，将筛选后的行合并到df_cur中   
    df_cur_spc = pd.concat([df_cur_spc, df_T_spc])

    # 按TS列排序  
    df_cur_spc.sort_values(by='DataTime', inplace=True)    
    # 计算最近60分钟的时间范围  
    recent_60min = now - pd.Timedelta('60min')    
    # 筛选最近60分钟内的行  
    mask_recent_60min = df_cur_spc['DataTime'] >= recent_60min  
    df_cur_spc = df_cur_spc[mask_recent_60min]
...
def do_etl_process(input_jsonify: string) -> dict:
    ...
    #读取blob文件
    blob_name_rawdata_spc = os.path.join(file_path,"CG_Sheeting_spc.csv")
    blob_name_rawdata_parm = os.path.join(file_path,"CG_Sheeting_HHData.csv")
    blob_name_curdata_spc = os.path.join(file_path_T1,"curated_spc.csv")
    ...
    spc_data = read_csv_blob_to_dataframe(connect_str, container_name_rawdata, blob_name_rawdata_spc, df_spc_empty)
    curated_data_parm = read_csv_blob_to_dataframe(connect_str, container_name_curateddata, blob_name_curdata_parm, df_parm_empty)
    curated_data_spc = read_csv_blob_to_dataframe(connect_str, container_name_curateddata, blob_name_curdata_spc, df_spc_empty)

    #######################开始etl处理######################
    #处理spc表 
    if not spc_data.empty:
        spc_data['DataTime'] = pd.to_datetime(spc_data['DataTime'])
    if not curated_data_spc.empty:
        curated_data_spc['DataTime'] = pd.to_datetime(curated_data_spc['DataTime'])   
    curated_data_spc = spc_cur_process(now, spc_data, curated_data_spc)
    #保存curated_data成blob文件到T时刻文件路径
    blob_name_curdata_spc = os.path.join(file_path, "curated_spc.csv")
    save_dataframe_to_csv_blob(connect_str, container_name_curateddata, blob_name_curdata_spc, curated_data_spc, index=False)
